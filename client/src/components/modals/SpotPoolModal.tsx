        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          <details className="mb-4 rounded-xl border border-slate-100 bg-slate-50/50 p-3">
            ...
            <div className="mt-3 flex flex-wrap items-end gap-3">
              ...
              <Field label="景点名称 *" className="min-w-[240px] flex-1">
                <PlaceAutoComplete
                  city={sortedCities.find((c) => c.id === spotCityId)?.name}
                  value={spotName}
                  onChange={setSpotName}
                  onPick={(v) => {
                    if (typeof v.lat === 'number' && typeof v.lng === 'number') {
                      setSpotLat(v.lat.toFixed(6))
                      setSpotLng(v.lng.toFixed(6))
                    }
                    if (v.address && !description.trim()) {
                      setDescription(v.address)
                    }
                  }}
                  placeholder="输入景点名即可联想，或手动输入后在地图右键取坐标"
                />
              </Field>
